...
INSURANCE_LINK = "https://heymondo.it/?utm_medium=Afiliado&utm_source=30SECONDSTOGUIDE&utm_campaign=PRINCIPAL&cod_descuento=30SECONDSTOGUIDE&ag_campaign=INPUT&agencia=JzPWeAXXi7s0b94oPYh2FmTwaWKFpiCp1a8PkjOn&redirect=TEMPORAL"
        ...
        if secret_pwd == "fabio123":
            ...
            if logs:
                for log in reversed(logs):
                    # Gestione sia per vecchi log (stringhe) che nuovi (dizionari)
...
with st.container():
    ...
    if st.button("✨ Crea il mio Travel Plan", type="primary", use_container_width=True, disabled=is_generated):
        if not destination:
            ...
            # --- SALVATAGGIO STATISTICHE DETTAGLIATE ---
            log_entry = {
                "timestamp": timestamp,
